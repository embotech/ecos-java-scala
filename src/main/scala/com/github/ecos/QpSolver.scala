...
object QpSolver {
  def main(args: Array[String]) {
    ...
    println("Test ecos QpSolver, breeze LBFGS and jblas solvePositive with problemSize " + problemSize)
    ...
    assert(norm(DenseVector(qpResult) - DenseVector(dposvResult), 2) < 1E-2)
    assert(norm(DenseVector(qpResultDiag) - DenseVector(dposvResult), 2) < 1E-2)
