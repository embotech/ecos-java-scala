...
package com.github.ecos.factorization

import breeze.linalg.{CSCMatrix, DenseVector}
import com.github.ecos.{QpSolver, factorization}
...
/**
  ...
  */
private[factorization] case class OutLinkBlock(elementIds: Array[Int], shouldSend: Array[BitSet])
/**
  ...
  */
private[factorization] case class InLinkBlock(
...
class ALS private (
  ...
  def run(ratings: RDD[Rating]): MatrixFactorizationModel = {
    ...
    val partitioner = new Partitioner {
      ...
      def getPartition(x: Any): Int = {
        nonNegativeMod(byteswap32(x.asInstanceOf[Int]), numPartitions)
      }
    }
    ...
    val ratingsByProductBlock = ratings.map { rating =>
      (partitioner.getPartition(rating.product),
        factorization.Rating(rating.product, rating.user, rating.rating))
    }
  ...
  private def updateBlock(messages: Iterable[(Int, Array[Array[Double]])], inLinkBlock: InLinkBlock,
                          rank: Int, lambda: Double, lambdaL1: Double, alpha: Double,
                          YtY: Option[Broadcast[DoubleMatrix]]): Array[Array[Double]] = {
    // Sort the incoming block factor messages by block ID and make them an array
    val blockFactors = messages.toSeq.sortBy(_._1).map(_._2).toArray
    // Array[Array[Double]]
    ...
    logInfo(s"qpProblem ${qpProblem} qpTime ${qpSolver.solveTime / 1e6}")
    factors
  }
